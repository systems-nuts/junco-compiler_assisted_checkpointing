...
static cl::opt<std::string> InjectionOption(
    "inject", cl::desc("Specify subroutine injection option"),
    cl::value_desc("option"));

static cl::opt<bool> TrackIndexOption(
    "trackingIndex", cl::desc("activate tracking indexes optimization"),
...
static RegisterPass<SubroutineInjection> X(
    "subroutine-injection", "Subroutine Injection",
...
bool SubroutineInjection::runOnModule(Module &M) {
    ...
    LiveValues::FullLiveValsInfo fullLiveValsInfo =
        JsonHelper::getFuncBBLiveValsInfo(FuncValuePtrs, FuncBBLiveValsByName,
                                          M);
...
void SubroutineInjection::printTrackedValues(
    ...
    for (funcIt = LVResult.cbegin(); funcIt != LVResult.cend(); funcIt++) {
        ...
        for (bbIt = bbTrackedVals->cbegin(); bbIt != bbTrackedVals->cend();
            ...
            O << "\n  Tracked:\n    ";
            for (valIt = trackedVals.cbegin(); valIt != trackedVals.cend();
                 valIt++) {
...
bool SubroutineInjection::injectSubroutines(
    ...
    if (func_mem_cpy_wrapper_f == NULL) {
        std::cout
            << "External mem_cpy_wrapper function CANNOT be found. Disable "
               "index tracking optimization."
            << std::endl;
        TrackIndexOption = false;
    }
    ...
    if (func_mem_cpy_bitcast_f == NULL) {
        std::cout
            << "External mem_cpy_bitcast_f function CANNOT be found. Disable "
               "index tracking optimization."
            << std::endl;
        TrackIndexOption = false;
    }
    ...
    for (auto &F : M.getFunctionList()) {
        ...
        if (F.getLinkage() == F.LinkOnceODRLinkage) continue;
        ...
        // re-calculate variableDefMap for func (could include alloca-ed vals
        // that are not part of live-in/out sets)
        ...
        Value *ckptMemSegment =
            getSelectedFuncParam(funcParams, segmentName, &M);
        if (!ckptMemSegment) {
            std::cout
                << "WARNING: Could not get pointer to memory segment of name '"
        ...
        Type *ckptMemSegContainedType =
            ckptMemSegment->getType()->getContainedType(
      ...
      */
        ...
        filteredBBTrackedVals =
            removeBBsWithNoTrackedVals(filteredBBTrackedVals);
        ...
        for (auto iter : bbCheckpoints) {
            ...
            for (auto iterr : iter.second) {
                std::cout << "  " << JsonHelper::getOpName(iterr, &M)
                          << std::endl;
            }
        }
        std::cout << "===========================\n\n" << std::endl;
        ...
        if (bbCheckpoints.size() == 0) {
            ...
            std::cout
                << "WARNING: Could not find any valid BBs with checkpoint "
      ...
      */
        ...
        if (InjectionOption == RESTORE_ONLY ||
            ...
            restoreControllerBB =
                splitEdgeWrapper(entryBB, restoreControllerSuccessor,
                                 restoreControllerBBName, M);
            if (restoreControllerBB) {
                isModified = true;
                restoreControllerSuccessor =
                    restoreControllerBB->getSingleSuccessor();
                std::cout << "successor of restoreControllerBB="
                          << JsonHelper::getOpName(restoreControllerSuccessor,
                                                   &M)
                          << "\n";
            } else {
                // Split-edge fails for adding BB after function entry block =>
                // skip this function
                std::cout
                    << "WARNING: Split-edge for restoreControllerBB failed for "
                       "function '"
                    << funcName << "'" << std::endl;
                continue;
            }
        ...
        for (auto bbIter : checkpointBBPtrSet) {
          ...
          */
            for (auto succIter : checkpointBBSuccessorsList) {
                ...
                /*
                ++ 3.2: For each successful saveBB, add restoreBBs and
                junctionBBs
                +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
              */
                std::string checkpointBBName =
                ...
                if (InjectionOption == RESTORE_ONLY ||
                    InjectionOption == SAVE_RESTORE) {
                    // create mediator BB as junction to combine output of
                    // saveBB and restoreBB
                    if (InjectionOption == SAVE_RESTORE) {
                        junctionBB = splitEdgeWrapper(
                            saveBB, resumeBB, checkpointBBName + ".junctionBB",
                            M);
                    } else {
                        junctionBB = splitEdgeWrapper(
                            checkpointBB, resumeBB,
                    ...
                    if (junctionBB) {
                        ...
                        restoreBB = BasicBlock::Create(
                            context, checkpointBBName + ".restoreBB", &F,
                            nullptr);
                        // have successfully inserted all components (BBs) of
                        // subroutine
                ...
                /*
                ++ 3.3: Populate saveBB and restoreBB with load and store
                instructions.
                +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
              */

                /** DALEJX: the code breaks here during restore-only
                 * transformation. Temporarily commented out for testing.
                 * Maxime:
                 * "If I remember correctly, the checkpoint instruction in the
                 * source code should be surrounded by scope.entry and
                 * scope.exit HLS directives. The we get this instruction in the
                 * pass and move them into the actual checkpoint to ensure
                 * in-order execution. (And we remove them from the main
                 * function)."
                 */
                // if (InjectionOption == RESTORE_ONLY)
                // {
                ...
                std::set<const Value *> trackedVals =
                    oldNewTrackedValsSetsPair.second;

                // sort tracked vals set by val name for consistent access later
                auto cmp = [&](const Value *a, const Value *b) {
                    std::string aName =
                        JsonHelper::getOpName(a, &M).erase(0, 1);
                    std::string bName =
                        JsonHelper::getOpName(b, &M).erase(0, 1);
                ...
                if (TrackIndexOption) {
                    allocateindexStacks(trackedVals, oldNewTrackedVals,
                                        allTrackedValVersions, valDefMap,
                                        liveValDefMap, ckptMemSegment, F, M,
                ...
                int valMemSegIndex =
                    VALUES_START;  // start index of "slots" for values in
                                   // memory segment
                for (auto iter : trackedValsOrdered) {
                    ...
                    -----------------------------------------------------------------------------
                  */
                    /** TODO: verify safety of cast to non-const!! this is
                     * dangerous*/
                    Value *trackedVal = const_cast<Value *>(
                        &*iter);  // is the current version of tracked
                    ...
                    Type *containedType = isPointer
                                              ? valRawType->getContainedType(0)
                                              : valRawType;
                    bool isPointerPointer =
                        isPointer && containedType->isPointerTy();

                    // init store location (index) in memory segment:
                    Value *indexList[1] = {ConstantInt::get(
                        Type::getInt32Ty(context), valMemSegIndex)};
                    // init valSize and numOfArrSlotsUsed for case where Value
                    ...
                    if (isPointer) {
                        if (containedType->isArrayTy()) {
                            ...
                            Value *trackedValDeref = getDerefValFromPointer(
                                originalTrackedVal, valVersions, &F);
                            ...
                            if (trackedValDeref != nullptr) {
                                if (valDefMap.count(trackedValDeref)) {
                                    // get number of ckpt_mem array slots used
                                    // to store this element:
                                    valSizeBytes =
                                        valDefMap.at(trackedValDeref);
                                    numOfArrSlotsUsed = ceil(
                                        (float)valSizeBytes /
                                        (float)ckptMemSegContainedTypeSize);
                                } else {
                                    // val has not been alloca-ed for in this
                                    // function => means it is of "primitive"
                                    // type
                                    numOfArrSlotsUsed = 1;
                                }
                            } else {
                                std::cout << "WARNING: Could not dereference'"
                                          << valName
                                          << "'; ignoring tracked value!"
                                          << std::endl;
                                continue;
                            }
                        }
                    }
                    // if valSizeBytes was 1, we "sign extend" it to fill up the
                    // available byte width of the ckpt mem segment.
                    ...
                    int paddedValSizeBytes = (valSizeBytes < sizeInCkptMemArr)
                                                 ...
                                                 : valSizeBytes;
                    printf(
                        "paddedValSizeBytes = %d, ckptMemSegContainedTypeSize "
                        "= %d\n",
                    ...
                    if (InjectionOption == SAVE_ONLY ||
                        InjectionOption == SAVE_RESTORE) {
                        /*
                        --- 3.3.3: Create instructions to store value to memory
                        segment
                        -----------------------------------------------------------------------------
                        */
                        Instruction *saveBBTerminator = saveBB->getTerminator();
                        Instruction *elemPtrStore =
                            GetElementPtrInst::CreateInBounds(
                                ckptMemSegContainedType, ckptMemSegment,
                                ArrayRef<Value *>(indexList, 1),
                                "idx_" + valName, saveBBTerminator);
                        Value *storeLocation = trackedVal;

                        if (isPointer) {
                            if (containedType->isArrayTy()) {
                                auto type =
                                    IntegerType::getInt8Ty(F.getContext());
                                Value *globalSync = M.getOrInsertGlobal(
                                    StringRef("sync_bit"), type);
                                if (globalSync != NULL) {
                                    auto vZero = llvm::ConstantInt::get(
                                        Type::getInt8Ty(F.getContext()), 0);
                                    StoreInst *storeSync =
                                        new StoreInst(vZero, globalSync, false,
                                                      saveBBTerminator);
                                }
                                ...
                                if (TrackIndexOption) {
                                    ...
                                    if (ite != stacksMem.end()) {
                                        ...
                                        // void mem_cpy_index_f(float* dest,
                                        // float* src, int* index_list, int* sp)
                                        CallInst *call1 = CallInst::Create(
                                            func_mem_cpy_index_f, call_params,
                                            "", saveBBTerminator);
                                        // Instruction *resetIdx =
                                        //     new
                                        //     StoreInst(llvm::ConstantInt::get(
                                ...
                                if (!copy_done) {
#ifndef LLVM14_VER
                                    auto srcAlign = DL.getPrefTypeAlignment(
                                        storeLocation->getType());
                                    auto dstAlign = DL.getPrefTypeAlignment(
                                        elemPtrStore->getType());
#else
                                    MaybeAlign srcAlign = DL.getPrefTypeAlign(
                                        storeLocation->getType());
                                    MaybeAlign dstAlign = DL.getPrefTypeAlign(
                                        elemPtrStore->getType());
#endif
                                    ...
                                    storeLocation, paddedValSizeBytes, srcAlign,
                                    true); #else CallInst *memcpyCall =
                                    builder.CreateMemCpy(reinterpret_cast<Value*>(elemPtrStore),
                                    dstAlign, storeLocation, srcAlign,
                                    paddedValSizeBytes, true); #endif
                                    */
                                    ...
                                    CallInst *call1 = CallInst::Create(
                                        func_mem_cpy_wrapper_f, call_params, "",
                                        saveBBTerminator);
...
// create memcpy inst (autoconverts pointers to i8*)
                                    ...
                                    IntegerType::getInt8Ty(F.getContext());
                                    Value* globalSync =
                                    M.getOrInsertGlobal(StringRef("sync_bit"),
                                    type); if(globalSync != NULL){ auto vOne =
                                    llvm::ConstantInt::get(Type::getInt8Ty(F.getContext()),
                                    1); StoreInst *storeSync = new
                                    StoreInst(vOne, globalSync, false,
                                    saveBBTerminator);
                                    }
                                    */
                                ...
                                // save dereferenced value by first loading
                                // value from the pointer
                                Instruction *deref =
                                    new LoadInst(containedType, storeLocation,
                                                 "deref_" + valName, false,
                                                 saveBBTerminator);
                                if (ckptMemSegContainedType != containedType) {
                                    std::string name =
                                        JsonHelper::getOpName(
                                            reinterpret_cast<Value *>(deref),
                                            &M)
                                            .erase(0, 1);
                                    deref = addTypeConversionInst(
                                        deref, ckptMemSegContainedType, name,
                                        saveBBTerminator);
                                }

                                Instruction *storeInst =
                                    new StoreInst(deref, elemPtrStore, false,
                                                  saveBBTerminator);
                            }
                        } else {
                            // do direct store
                            if (ckptMemSegContainedType !=
                                storeLocation->getType()) {
                                Instruction *typeConv = addTypeConversionInst(
                                    storeLocation, ckptMemSegContainedType,
                                    valName, saveBBTerminator);
                                StoreInst *storeInst =
                                    new StoreInst(typeConv, elemPtrStore, false,
                                                  saveBBTerminator);
                            } else {
                                StoreInst *storeInst =
                                    new StoreInst(storeLocation, elemPtrStore,
                                                  false, saveBBTerminator);
                            }
                        }
                        // synthesize liveness data with both updated and
                        // original versions of tracked values:
                    ...
                    if (InjectionOption == RESTORE_ONLY ||
                        InjectionOption == SAVE_RESTORE) {
                        /*
                        --- 3.3.4: Create instructions to load value from
                        memory.
                        -----------------------------------------------------------------------------
                      */
                        Instruction *restoreBBTerminator =
                            restoreBB->getTerminator();
                        Value *restoredVal = nullptr;
                        Instruction *elemPtrLoad =
                            GetElementPtrInst::CreateInBounds(
                                ckptMemSegContainedType, ckptMemSegment,
                                ArrayRef<Value *>(indexList, 1),
                                "idx_" + valName, restoreBBTerminator);
                        Value *storeLocationOrig =
                            originalTrackedVal;  // is where the original value
                                                 // was stored during save
                                                 // operation
                        if (isPointer) {
                            if (containedType->isArrayTy()) {
...
// restore values back into original pointer
                                ...
                                CallInst *memcpyCallOrig = builder.CreateMemCpy(
                                    storeLocationOrig,
                                    reinterpret_cast<Value *>(elemPtrLoad),
                                    paddedValSizeBytes, srcAlignOriginalPtr,
                                    true);
#else
                                CallInst *memcpyCallOrig = builder.CreateMemCpy(
                                    storeLocationOrig, dstAlignOriginalPtr,
                                    reinterpret_cast<Value *>(elemPtrLoad),
                                    srcAlignOriginalPtr, paddedValSizeBytes,
                                    true);
#endif
                                restoredVal = nullptr;    // do not propagate
                                                          ...
                                                          // 1)
                            {
                                // at this point, allocaInstR has type <type>**
                                // (is allocated memory for array.addr)

                                /** TODO: ----- memcpy new (restored) array back
                                 * into the original array pointer ----- */
                                // place inst in restoreBB to load array
                                // base-address (<type>*) from
                                // originalTrackedVal (<type>**) into "local"
                                // Value
                                Instruction *loadedAddrSOrig = new LoadInst(
                                    containedType, storeLocationOrig,
                                    "loaded_" + valName, false,
                                    restoreBBTerminator);
                                ...
                                /** TODO: The following store inst is
                                 * unnecessary since we're using the original
                                 * array address */
                                // store <type>* into original the <type>**
                                // Value (i.e. originalTrackedVal) pointing to
                                // the array StoreInst *storeInst = new
                                // StoreInst(storeLocationOrig,
                                // originalTrackedVal, false,
                                // restoreBBTerminator);
                                restoredVal = nullptr;  // do not propagate
                            } else {
                                /** TODO: Choose between 1) propagating
                                 * newly-allocated single-ptr OR 2) storing back
                                 * to original val */

                                /** ----- Option 1: newly-allocate single-ptr
                                 * and propgate new value ----- */
                                // allocate memory (ptr) to store value into
                                AllocaInst *allocaInstR = new AllocaInst(
                                    containedType, 0, "alloca_" + valName,
                                    restoreBBTerminator);

                                // load value from memory and store into
                                // alloca-ed mem
                                Instruction *loadInst = new LoadInst(
                                    ckptMemSegContainedType, elemPtrLoad,
                                    "load_derefed_" + valName, false,
                                    restoreBBTerminator);
                                if (ckptMemSegContainedType != containedType) {
                                    std::string name =
                                        JsonHelper::getOpName(
                                            reinterpret_cast<Value *>(loadInst),
                                            &M)
                                            .erase(0, 1);
                                    loadInst = addTypeConversionInst(
                                        loadInst, containedType, name,
                                        restoreBBTerminator);
                                }
                                // store <type>* into new <type>** to propagate
                                // through CFG
                                StoreInst *storeInst =
                                    new StoreInst(loadInst, allocaInstR, false,
                                                  restoreBBTerminator);
                                restoredVal = allocaInstR;

                                /** ----- Option 2: store new (restored) value
                                 * back into the original pointer ----- */
                                /** TODO: removing this option will somehow
                                 * cause a "instructoin does not dominate"
                                 * error*/
                                // StoreInst *storeInstOriginal = new
                                // StoreInst(loadInst, storeLocationOrig, false,
                                // restoreBBTerminator); restoredVal = nullptr;
                            }
                        } else {
                            // do direct load
                        ...
                        /*
                        --- 3.3.5: Add phi node into junctionBB to merge loaded
                        val & original val
                        -----------------------------------------------------------------------------
                      */
                        if (restoredVal !=
                            nullptr)  // do not add phi node if we choose not to
                                      // propagate
                        {
                            PHINode *phi = PHINode::Create(
                                trackedVal->getType(), 2, "new_" + valName,
                        ...
                        /*
                        --- 3.3.6: Configure live-out sets for saveBB, restoreBB
                        and junctionBB; init trackValPhiValMap.
                        -----------------------------------------------------------------------------
                      */
                        /* Since the live-out data for all other BBs use the
                        original value version too (and algo checks live-out
                        using this live-out data), it would be more consistent
                        to use original value version as live-out of saveBB,
                        restoreBB & junctionBB instead of the new phi. */
                        // synthesize liveness data with both updated and
                        // original versions of tracked values:
                  ...
                  */
                    for (auto iter : oldNewTrackedVals) {
                        ...
                        if (valType->isPointerTy()) {
                            /** TODO: do 'continue' here if not propagating all
                             * pointer types*/
                            // continue;
                            // do not propagate <type>** Values
                            if (valType->getContainedType(0)->isPointerTy())
                                continue;
                            // do not propagate llvm array pointer type Values
                            // (ptr to [<size> x <type>] arr)
                            if (valType->getContainedType(0)->isArrayTy())
                                continue;
                        }

                        // for each BB, keeps track of versions of values that
                        // have been encountered in this BB (including the
                        // original value)
                        std::map<BasicBlock *, std::set<Value *>> visitedBBs;

                        // get phi value in junctionBB that merges original &
                        // loaded versions of trackVal
                        if (!funcJunctionBBPhiValsMap.at(junctionBB)
                                 .count(trackedVal)) {
                            std::cout << "WARNING: No PHI node inserted in "
                                         "junctionBB for "
                                         "tracked val"
                        ...
                        PHINode *phi = funcJunctionBBPhiValsMap.at(junctionBB)
                                           .at(trackedVal);

                        propagateRestoredValuesBFS(
                            resumeBB, junctionBB, trackedVal, phi,
                            originalTrackedVal, &visitedBBs, &funcBBLiveValsMap,
                            funcSaveBBsLiveOutMap, funcRestoreBBsLiveOutMap,
                            funcJunctionBBsLiveOutMap, &bbCheckpointsOldNewVals,
            ...
            // break; /** TODO: IS FOR TESTING (limits to 1 checkpoint;
            // propagation algo does not work with > 1 ckpt) */
        }
      ...
      */
        ...
        for (auto iter : ckptIDsCkptToposMap) {
          ...
          */
            ...
            if (InjectionOption == SAVE_ONLY ||
                ...
                Value *memLockCkptIDInt = {ConstantInt::get(
                ...
                if (ckptMemSegContainedType != Type::getInt32Ty(context)) {
                    Value *memLockCkptIDValFloat = addTypeConversionInst(
                        memLockCkptIDInt, ckptMemSegContainedType,
                        "mem_lock_ckpt_id", firstNonPhiInstSaveBB);
                    savedMemLockCkptIDVal = memLockCkptIDValFloat;

                    Value *ckptIDValFloat = addTypeConversionInst(
                        ckptIDValInt, ckptMemSegContainedType, "ckpt_id",
                        saveBBTerminator);
                    savedCkptIDVal = ckptIDValFloat;
                }
                /*
                StoreInst *storeMemLockCkptID = new
                StoreInst(savedMemLockCkptIDVal, elemPtrCkptId, false,
                firstNonPhiInstSaveBB); StoreInst *storeCkptId = new
                StoreInst(savedCkptIDVal, elemPtrCkptId, false,
                saveBBTerminator);
                */
                ...
                if ((instScopeEntry != NULL) && ((instScopeExit != NULL))) {
                    ...
                    if (globalSync != NULL) {
                        ...
                        StoreInst *storeSync = new StoreInst(
                            vzero, globalSync, false, elemPtrCkptId);
                    }
                    ...
                    StoreInst *storeMemLockCkptID =
                        new StoreInst(savedMemLockCkptIDVal, elemPtrCkptId,
                                      false, firstNonPhiInstSaveBB);
                    StoreInst *storeCkptId = new StoreInst(
                        savedCkptIDVal, elemPtrCkptId, false, saveBBTerminator);
          ...
          */
            ...
            Value *addRhsOperandInt =
                ConstantInt::get(Type::getInt32Ty(context), 1);
            if (InjectionOption == SAVE_ONLY ||
                InjectionOption == SAVE_RESTORE) {
                // add inst to saveBB
                Instruction *elemPtrHeartbeatS =
                    GetElementPtrInst::CreateInBounds(
                        ckptMemSegContainedType, ckptMemSegment,
                ...
                if (!ckptMemSegContainedType->isIntegerTy()) {
                    ...
                    loadHeartbeatS = addTypeConversionInst(
                        loadHeartbeatS, Type::getInt32Ty(context), "heartbeat",
                        saveBBTerminator);
                }
                builder.SetInsertPoint(saveBBTerminator);
                Value *addInstS = builder.CreateAdd(
                    loadHeartbeatS, addRhsOperandInt, "heartbeat_incr");
                if (!ckptMemSegContainedType->isIntegerTy()) {
                    // convert int to float for storage
                    addInstS = addTypeConversionInst(
                        addInstS, ckptMemSegContainedType, "heartbeat_incr",
                        saveBBTerminator);
                }
                StoreInst *storeHeartBeatS = new StoreInst(
                    addInstS, elemPtrHeartbeatS, false, saveBBTerminator);
            ...
            if (InjectionOption == RESTORE_ONLY ||
                ...
                if (!ckptMemSegContainedType->isIntegerTy()) {
                    // convert float to int for addition
                    loadHeartbeatR = addTypeConversionInst(
                        loadHeartbeatR, Type::getInt32Ty(context), "heartbeat",
                        restoreBBTerminator);
                }
                builder.SetInsertPoint(restoreBBTerminator);
                Value *addInstR = builder.CreateAdd(
                    loadHeartbeatR, addRhsOperandInt, "heartbeat_incr");
                if (!ckptMemSegContainedType->isIntegerTy()) {
                    // convert int to float for storage
                    addInstR = addTypeConversionInst(
                        addInstR, ckptMemSegContainedType, "heartbeat_incr",
                        restoreBBTerminator);
                }
                StoreInst *storeHeartBeatR = new StoreInst(
                    addInstR, elemPtrHeartbeatR, false, restoreBBTerminator);
            }
        }
        ...
        if (InjectionOption == RESTORE_ONLY ||
            ...
            =============================================================================
          */
            /* a. if CheckpointID indicates no checkpoint has been saved,
              continue to computation. b. if CheckpointID exists, jump to
              restoreBB for that CheckpointID. */
            ...
            LoadInst *loadCheckpointID =
                new LoadInst(ckptMemSegContainedType, elemPtrLoad,
                             "load_ckpt_id", false, terminatorInst);
            Value *intCkptId = loadCheckpointID;
            // convert loaded ckpt id into int32
            if (!ckptMemSegContainedType->isIntegerTy()) {
                intCkptId = addTypeConversionInst(loadCheckpointID,
                                                  Type::getInt32Ty(context),
          ...
          */
            ...
            SwitchInst *switchInst = builder.CreateSwitch(
                intCkptId, restoreControllerSuccessor, numCases);
            switchInst->removeFromParent();
            // printf("p terminatorInst %x p switchInst %x\n",
            // terminatorInst->getParent(), switchInst->getParent());
            // assert(switchInst->getParent() == nullptr &&
            // "ReplaceInstWithInst: Instruction already inserted into basic
            // block!");
      ...
      */
            ...
            for (auto bbIter = BB->begin(); bbIter != BB->end(); ++bbIter) {
                ...
                if (isa<ReturnInst>(Inst)) {
                    ...
                    if (Inst->getNumOperands() == 0) {
                        ...
                        /** TODO:
                         * Storing return value will cause current pass to throw
                         * segfault if retVal is a pointer. Is cuz pointer
                         * cannot be type-converted using simple instructions.
                         */
                        Value *p_retVal =
                            dyn_cast<ReturnInst>(Inst)->getReturnValue();
                        isComplete =
                            (p_retVal->getType()->isPointerTy())
                                ? ConstantInt::get(Type::getInt32Ty(context), 1)
                                : p_retVal;
                    }

                    // insert inst into saveBB
                    Instruction *elemPtrIsCompleteS =
                        GetElementPtrInst::CreateInBounds(
                            ckptMemSegContainedType, ckptMemSegment,
        ...
        /* =============================================================================
         */
        // store map of ckpt sizes; done only after all ckpting infrastructure
        // is completed
...
void SubroutineInjection::propagateRestoredValuesBFS(
    ...
    while (!q.empty()) {
        ...
        processUpdateRequest(updateRequest, &q, originalTrackedVal, visitedBBs,
                             ...
                             funcRestoreBBsLiveOutMap,
                             funcJunctionBBsLiveOutMap, ckptBBOldNewValsMap);
...
void SubroutineInjection::processUpdateRequest(
    ...
    // BB.
    std::set<Value *> bbValueVersions = getOrDefault(
        currBB, visitedBBs);  // marks BB as visited (if not already)
    // stop propagation if val versions in valueVersions and bbValueVersions
    // match exactly.
    ...
    if (hasNPredecessorsOrMore(currBB, 2) &&
        1 < numOfPredsWhereVarIsLiveOut(
                currBB, originalTrackedVal, funcBBLiveValsMap,
                funcSaveBBsLiveOutMap, funcRestoreBBsLiveOutMap,
                funcJunctionBBsLiveOutMap)) {
        if (isPhiInstExistForIncomingBBForTrackedVal(valueVersions, currBB,
            ...
            for (auto phiIter = currBB->phis().begin();
                ...
                for (unsigned i = 0; i < phi->getNumIncomingValues(); i++) {
                    ...
                    if (incomingBB == prevBB &&
                        valueVersions.count(incomingValue)) {
                        if (incomingValue == newVal) {
                            // we've already updated this phi instruction to use
                            // newVal in a previous traversal path do not add
                            // successors to BFS queue again.
                            continue;
                        } else {
                            ...
                            bbValueVersions.insert(
                                valueVersions.begin(),
                                valueVersions
                                    .end());  // copy contents of valueVersions
                                              // into bbValueVersions
                            updateMapEntry(currBB, bbValueVersions, visitedBBs);

                            std::string phiName = JsonHelper::getOpName(phi, M);
                            std::string incomingBBName =
                                JsonHelper::getOpName(incomingBB, M);
                            std::string valueName =
                                JsonHelper::getOpName(incomingValue, M);
                            std::string newValName =
                                JsonHelper::getOpName(newVal, M);
                            std::cout << "modify " << phiName << ": change ["
                                      << valueName << ", " << incomingBBName
                                      << "] to [" << newValName << ", "
                                      << incomingBBName << "]\n";
                        }
                    }
            ...
            // Do not propagate the LHS of the modified phi node further through
            // the cfg If it's a new phi that algo has added, it should already
            // have been propagated by the "Add new PHI" block. If it's an
            // existing phi that was part of the CFG before propagation, then
            // the phi value should already be in the correct places in the cfg
            // and does not need to be re-propagtaed.
        } else {
            std::cout << "ADD NEW PHI NODE\n";
            // make new phi node
            std::string bbName = JsonHelper::getOpName(currBB, M).erase(0, 1);
            std::string newValName =
                JsonHelper::getOpName(newVal, M).erase(0, 1);
            std::vector<BasicBlock *> predecessors = getBBPredecessors(currBB);
            Instruction *firstInst = &*(currBB->begin());
            PHINode *newPhi =
                PHINode::Create(oldVal->getType(), predecessors.size(),
            ...
            for (BasicBlock *predBB : predecessors) {
                // if pred has exit edge to startBB and val is live-out from it,
                // add new entry in new phi instruction.
                if (isValLiveOutofBB(predBB, originalTrackedVal,
                                     funcBBLiveValsMap, funcSaveBBsLiveOutMap,
                                     funcRestoreBBsLiveOutMap,
                                     funcJunctionBBsLiveOutMap)) {
                    Value *phiInput = (predBB == prevBB) ? newVal : oldVal;
                    std::cout << "  add to phi: {"
                              << JsonHelper::getOpName(phiInput, M) << ", "
            ...
            // update each subsequent instruction in this BB from oldVal to
            // newPhi
            for (auto instIter = currBB->begin(); instIter != currBB->end();
                 instIter++) {
                Instruction *inst = &*instIter;
                if (inst != dyn_cast<Instruction>(
                                newPhi))  // don't update new phi instruction
                {
                ...
                if (valueVersions.count(inst))
                    isStop = true;  // inst is a definition of one of the value
                                    // versions.
            }
            ...
            bbValueVersions.insert(
                ...
                valueVersions.end());  // copy contents of valueVersions into
                                       // bbValueVersions
            updateMapEntry(currBB, bbValueVersions, visitedBBs);
            // replace old value in ckptBBMap with this newPhi (if curBB is part
            // of ckptBBOldNewValsMap)
            if (ckptBBOldNewValsMap->count(currBB))
                updateCkptBBMap(currBB, newPhi, bbValueVersions,
                                ckptBBOldNewValsMap, originalTrackedVal);

            if (!isStop) {
                // add direct successors of BB to queue (convert oldVal to
                // newPhi)
        ...
        for (auto instIter = currBB->begin(); instIter != currBB->end();
            ...
            if (valueVersions.count(inst))
                isStop =
                    true;  // inst is a definition of one of the value versions.
        }
        valueVersions.insert(newVal);
        bbValueVersions.insert(
            valueVersions.begin(),
        ...
        // ckptBBOldNewValsMap)
        if (ckptBBOldNewValsMap->count(currBB))
            updateCkptBBMap(currBB, newVal, bbValueVersions,
                            ckptBBOldNewValsMap, originalTrackedVal);
...
unsigned SubroutineInjection::numOfPredsWhereVarIsLiveOut(
    ...
    for (auto iter = pred_begin(BB); iter != pred_end(BB); iter++) {
        ...
        if (isValLiveOutofBB(pred, val, funcBBLiveValsMap,
                             funcSaveBBsLiveOutMap, funcRestoreBBsLiveOutMap,
                             funcJunctionBBsLiveOutMap)) {
            count++;
        }
...
void SubroutineInjection::updateCkptBBMap(
    ...
    for (auto val : bbValueVersions) {
        if (updatedTrackedVals.count(val)) {
            ...
            std::cout << ">> " << JsonHelper::getOpName(ckptBB, M)
                      << ": Replaced " << JsonHelper::getOpName(val, M)
                      << " with " << JsonHelper::getOpName(newVal, M)
                      << std::endl;
            break;  // there should only be one version of value in
                    // ckptBBTrackedVals set
        }
    }
}
...
void SubroutineInjection::updateAllTrackedValVersionsMap(
    ...
    if (allTrackedValVersions->count(originalTrackedVal)) {
        std::cout << "£££ add " << JsonHelper::getOpName(newTrackedVal, M)
                  << " to " << JsonHelper::getOpName(originalTrackedVal, M)
                  << " set" << std::endl;
        std::set<const Value *> *valVersionsSet =
            &allTrackedValVersions->at(originalTrackedVal);
...
SubroutineInjection::getCheckpointIdBBMap(
    ...
    int funcCkptIDCounter =
        startingCkptNum;  // id=0 means no ckpt has been saved
    ...
    for (iter = checkpointBBTopoMap.begin(); iter != checkpointBBTopoMap.end();
        ...
        if (saveBB != nullptr) {
            std::string saveBBName =
                JsonHelper::getOpName(saveBB, &M).erase(0, 1) + ".id" +
...
Instruction *SubroutineInjection::getCmpInstForCondiBrInst(
    Instruction *condiBranchInst, Module &M) const {
    ...
    while (cmp_instr == nullptr) {
        ...
        if ((isa<ICmpInst>(instr) || isa<FCmpInst>(instr)) &&
            instr == condition) {
            cmp_instr = instr;
        }
    }
...
std::set<BasicBlock *> SubroutineInjection::getCkptBBsInFunc(
    Function *F, CheckpointBBMap &bbCheckpoints) const {
    std::set<BasicBlock *> checkpointBBPtrSet;
...
std::set<Value *> SubroutineInjection::getFuncParams(Function *F) const {
    ...
    for (argIter = F->arg_begin(); argIter != F->arg_end(); argIter++) {
        Value *arg = &*argIter;
        StringRef argName =
            JsonHelper::getOpName(arg, F->getParent()).erase(0, 1);
...
Instruction *SubroutineInjection::addTypeConversionInst(
    Value *val, Type *destType, std::string valName,
    ...
    if ((val->getType()->isIntegerTy()) &&
        ...
        return CastInst::CreateFPCast(val, Type::getDoubleTy(bb->getContext()),
                                      "double_" + valName, insertBefore);
    } else if (val->getType()->isDoubleTy()) {
        if (destType->isIntegerTy()) {
            /** DALEJX: check if this actually works */
            return new FPToSIInst(val, destType, "double_" + valName,
                                  insertBefore);
        } else if (destType->isFloatTy()) {
            return CastInst::CreateFPCast(val,
                                          Type::getFloatTy(bb->getContext()),
                                          "float_" + valName, insertBefore);
        }
    } else {
...
std::vector<BasicBlock *> SubroutineInjection::getBBPredecessors(
    BasicBlock *BB) const {
...
std::vector<BasicBlock *> SubroutineInjection::getBBSuccessors(
    BasicBlock *BB) const {
...
std::vector<BasicBlock *> SubroutineInjection::getNonExitBBSuccessors(
    BasicBlock *BB) const {
    std::vector<BasicBlock *> BBSuccessorsList;
    ...
    for (auto &F : M.getFunctionList()) {
        ...
        for (bbIter = F.begin(); bbIter != F.end(); ++bbIter) {
            ...
            for (instrIter = bb->begin(); instrIter != bb->end(); ++instrIter) {
                ...
                for (operand = instrIter->op_begin();
                     operand != instrIter->op_end(); ++operand) {
...
long unsigned int SubroutineInjection::getMaxNumOfTrackedValsForBBs(
    ...
    auto maxElem =
        std::max_element(bbTrackedVals.cbegin(), bbTrackedVals.cend(),
...
SubroutineInjection::chooseBBWithLeastTrackedVals(
    ...
    // all BBs)
    auto minElem =
        std::min_element(bbTrackedVals.cbegin(), bbTrackedVals.cend(),
                         [=](const auto &a, const auto &b) {
                             // return true if a < b:
    ...
    if (minSize >= minValsCount) {
        ...
        for (bbIt = bbTrackedVals.cbegin(); bbIt != bbTrackedVals.cend();
            ...
            const BasicBlock *bb = bbIt->first;
            const std::set<const Value *> &trackedVals = bbIt->second;
            // get elements of trackedVals with min number of tracked values
            // that is at least minValCount
...
LiveValues::BBTrackedVals SubroutineInjection::removeSelectedTrackedVals(
    ...
    for (funcIter = bbTrackedVals.cbegin(); funcIter != bbTrackedVals.cend();
        ...
        for (auto valIter : trackedValues) {
            ...
            std::string valName =
                JsonHelper::getOpName(const_cast<Value *>(val), M);

            // if ignoredValues contains a value whose name is a substring in
            // valName
            // => ingore
            bool isMatchNameInIgnoredValues = false;
            for (auto ignoredVal : ignoredValues) {
                std::string ignoredValName =
                    JsonHelper::getOpName(ignoredVal, M) + ".";
            ...
            if (ignoredValues.count(const_cast<Value *>(
                    val))) /** TODO: verify safety of cast to
    ...
    for (funcIter = bbTrackedVals.cbegin(); funcIter != bbTrackedVals.cend();
        ...
        for (auto valIter : trackedValues) {
            ...
            if (valName.contains(matchStr) && valType->isPointerTy() &&
                ...
                std::cout << "Tracked value '" << JsonHelper::getOpName(val, M)
                          << "' in BB '" << JsonHelper::getOpName(BB, M)
                          << "' is a nested pointer type. Removed from "
                             "bbTrackedVals map."
                          << std::endl;
            } else {
                filteredTrackedValues.insert(val);
...
void SubroutineInjection::printCheckpointIdBBMap(
    ...
    for (iter = map.cbegin(); iter != map.cend(); ++iter) {
        ...
        std::cout << "RestoreBB = " << JsonHelper::getOpName(topo.restoreBB, M)
                  << "\n";
        std::cout << "JunctionBB = "
                  << JsonHelper::getOpName(topo.junctionBB, M) << "\n";
        std::cout << "\n";
    }
}
        ...
        for (instrIter = BB->begin(); instrIter != BB->end(); ++instrIter) {
            ...
            if (inst->getOpcode() == Instruction::Call ||
                ...
                if (name.contains("scope.entry")) {
                    std::cout << "\n contain scope entry\n";
                    std::cout
                        << ">>>>>>>>>>>> instruction name = " << name.str()
                        << std::endl;
                    instScopeEntry = inst;
                    // ensure that we have tracked-values information on the
                    // selected checkpoint BB
            ...
            if (inst->getOpcode() == Instruction::Call ||
                ...
                if (name.contains("checkpoint")) {
                    ...
                    for (bbIt = bbTrackedVals.cbegin();
                         bbIt != bbTrackedVals.cend(); bbIt++) {
                        const BasicBlock *trackedBB = bbIt->first;
                        const std::set<const Value *> &trackedVals =
                            bbIt->second;
                        std::cout << "  trackedBB="
                                  << JsonHelper::getOpName(trackedBB, M)
                                  << std::endl;
                        if (trackedBB == BB) {
                            std::cout << "\n BB added" << std::endl;
                            curr_BB_added = true;
...
std::tuple<llvm::Value *, Value *> SubroutineInjection::getOffsetArray(
    Value *v, Function &F) {
        ...
        for (instrIter = BB->begin(); instrIter != BB->end(); ++instrIter) {
            ...
            if (I == v) {
                ...
                if (I->getOpcode() == Instruction::GetElementPtr) {
                    Value *value;
                    std::tie(value, offset) =
                        getOffsetArray(I->getOperand(0), F);
    ...
    */
    ...
    for (BBIter = F.begin(); BBIter != F.end(); ++BBIter) {
        ...
        for (instrIter = BB->begin(); instrIter != BB->end(); ++instrIter) {
            ...
            if (I->getOpcode() == Instruction::Store) {
                ...
                if (offset != nullptr) {
                    ...
                    if (isa<AllocaInst>(value)) {
                        ...
                        for (auto it = stacksMem.begin(); it != stacksMem.end();
                             ++it)
                        ...
                        // %y = getelementptr inbounds i32, i32* %x, i32
                        // %idx_new
                        Value *IndexTrunc = BinaryOperator::Create(
                            Instruction::And, loadedAddr,
                            IR.getInt32(stackArraySizeMask), "", I);
                        Value *indexList[2] = {IR.getInt32(0),
                                               IndexTrunc};  // post inc
                        // Value *indexList[2] = {IR.getInt32(0), loadedAddr};
                        // Value *indexList[2] = {IR.getInt32(0),
                        // IR.getInt32(0)}; //pointer to base
                        Instruction *elemPtrStore =
                            GetElementPtrInst::CreateInBounds(
                        ...
                        if (offset->getType()->isIntegerTy()) {
                            if (offset->getType() != integer32T) {
                                offset_val = CastInst::CreateIntegerCast(
                                    offset, integer32T, true, "", I);
                            } else {
                                offset_val = offset;
                            }
                        } else if (offset->getType()->isFloatingPointTy()) {
                            offset_val =
                                CastInst::Create(Instruction::FPToSI, offset,
                        ...
                        << " param " << call_params[i]->getType()->getTypeID()
                        << std::endl;

                        CallInst* call1 = CallInst::Create(func_stack_push,
                        call_params, "", I);
                        */

                        Instruction *storeInst =
                            new StoreInst(offset_val, elemPtrStore, I);
                    } else if (isa<LoadInst>(value)) {
                        LoadInst *LI = cast<LoadInst>(value);
                        std::cout
                            << "value->getOperand(0) = " << LI->getOperand(0)
                            << std::endl;
                        ...
                        Value *indexList[2] = {IR.getInt32(0),
                                               IndexTrunc};  // post inc
                        // Value *indexList[2] = {IR.getInt32(0),
                        // IR.getInt32(0)}; //pointer to base Value
                        // *indexList[2] = {IR.getInt32(0), loadedAddr};
                        // //post inc
                        Instruction *elemPtrStore =
                            GetElementPtrInst::CreateInBounds(
                                ArrayTy, stack, ArrayRef<Value *>(indexList, 2),
                        ...
                        */
                        ...
                        std::cout
                            << "Insert index tracking optim failed for value "
                            << JsonHelper::getOpName(value, F.getParent())
                            << ": unrecognised instruction type.\n";
                        exit(1);
                    }
                } else {
            ...
            } else if (IntrinsicInst::classof(I) &&
                ...
                // trigger stack pointer overflow to force full-array copy in
                // mem_cpy_index()
                IRBuilder<> IR(I);
                Instruction *storeSP =
                    new StoreInst(IR.getInt32(stackArraySize + 1), index,
...
void SubroutineInjection::allocateindexStacks(
    ...
    Function &F, Module &M, Function *func_mem_cpy_bitcast_f) {
    const DataLayout &DL = M.getDataLayout();
    ...
    for (auto iter : trackedValsOrdered) {
        ...
        if (isPointer) {
            ...
            // allTrackedValVersions.at(originalTrackedVal); Value
            // *trackedValDeref = getDerefValFromPointer(trackedVal,
            // valVersions, &F);
            ...
            if (containedType->isArrayTy()) {
                ...
                // int arraySize = 1024*128; //int(valSizeBytes /
                // containedTypeSize);
                ...
                AllocaInst *allocaInst =
                    new AllocaInst(ArrayTy, 0, fullName, term);
                stacksMem.insert(
                ...
                stacksType.insert(
                    std::pair<Value *, Type *>(trackedVal, ArrayTy));
                ...
                std::vector<Value *> call_params;
                call_params.push_back(elemPtrStore);
                Value *castedSrc = CastInst::CreateIntegerCast(
                    reinterpret_cast<Value *>(storeLocation),
                    Type::getInt8PtrTy(F.getContext()), false,
                    "casted_" +
                        JsonHelper::getOpName(
                            reinterpret_cast<Value *>(storeLocation), &M)
                            .erase(0, 1),
                    term);
                ...
                CallInst *call1 = CallInst::Create(func_mem_cpy_bitcast_f,
                                                   call_params, "", term);

                // // create memcpy inst (autoconverts pointers to i8*)
                // #ifndef LLVM14_VER
                // auto srcAlign =
                // DL.getPrefTypeAlignment(storeLocation->getType()); auto
                // dstAlign = DL.getPrefTypeAlignment(elemPtrStore->getType());
                // #else
                // MaybeAlign srcAlign =
                // DL.getPrefTypeAlign(storeLocation->getType()); MaybeAlign
                // dstAlign = DL.getPrefTypeAlign(elemPtrStore->getType());
                // #endif
                ...
                if (trackedValDeref != nullptr) {
                    if (valDefMap.count(trackedValDeref)) {
                        ...
                        // containedTypeSize)
                        Type *ArrayTy = ArrayType::get(
                            Type::getInt32Ty(F.getContext()), stackArraySize);
                        std::string valName =
                            JsonHelper::getOpName(trackedValDeref, &M)
                                .erase(0, 1);
                        std::string fullName = valName + "_index_stack";
                        AllocaInst *allocaInst =
                            new AllocaInst(ArrayTy, 0, fullName, term);
                        stacksMem.insert(std::pair<Value *, Value *>(
                            trackedVal, allocaInst));
                        std::string IndexName = valName + "_index_sp";
                        auto index_size = llvm::ConstantInt::get(
                        ...
                        Instruction *storeInst = new StoreInst(
                            ...
                            allocaInstSP, term);
                        stacksIndex.insert(std::pair<Value *, Value *>(
                            trackedVal, allocaInstSP));
                        stacksType.insert(
                            std::pair<Value *, Type *>(trackedVal, ArrayTy));

                        // Initiate ckpt_mem with intial values (Required due to
                        // partial array checkpointing)
                        Value *storeLocation = trackedVal;
                        Instruction *loadedAddrS = new LoadInst(
                            valRawType->getContainedType(0), trackedVal,
                        ...
                        call_params.push_back(llvm::ConstantInt::get(
                            ...
                            paddedValSizeBytes));
                        CallInst *call1 = CallInst::Create(
                            func_mem_cpy_bitcast_f, call_params, "", term);
                        ...
                        // MaybeAlign dstAlign =
                        // DL.getPrefTypeAlign(elemPtrStore->getType()); #endif
                        // IRBuilder<> builder(F.getContext());
                        // builder.SetInsertPoint(term);
                        // #ifndef LLVM14_VER
                        // CallInst *memcpyCall = builder.CreateMemCpy(
