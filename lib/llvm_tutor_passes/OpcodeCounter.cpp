...
OpcodeCounter::Result OpcodeCounter::generateOpcodeMap(llvm::Function &Func) {
  ...
  // returns OpcodeMap as the Result.
  return OpcodeMap;
}

// Is the 1st analysis pass that generates the opcode map
OpcodeCounter::Result OpcodeCounter::run(llvm::Function &Func, llvm::FunctionAnalysisManager &) {
  return generateOpcodeMap(Func);
}

// Is the 2nd pass that is actually run by opt; returns PreservedAnalyses
PreservedAnalyses OpcodeCounterPrinter::run(Function &Func, FunctionAnalysisManager &FAM) {
  // "gets the result of an anaysis pass for the given IR unit"
  // runs OpcodeCounter::generateOpcodeMap first to get StringMap of results.
