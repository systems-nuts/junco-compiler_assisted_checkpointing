...
// License: MIT
//==============================================================================
#include "llvm_tutor_passes/MBASub.h"
...
//-----------------------------------------------------------------------------
// MBASub Implementaion (transformation pass)
//-----------------------------------------------------------------------------
bool MBASub::runOnBasicBlock(BasicBlock &BB) {
  bool Changed = false;

  // Loop over all instructions in the block. Replacing instructions requires
  // iterators, hence a for-range loop wouldn't be suitable. BasicBlock.begin() returns an iterator
  for (auto Inst = BB.begin(), IE = BB.end(); Inst != IE; ++Inst) { // does pre-increment of iterator

    // Skip non-binary (e.g. unary or compare) instruction.
    auto *BinOp = dyn_cast<BinaryOperator>(Inst); // gets an instruction pointer (binary operator instr)
    if (!BinOp)
      continue;
    ...
    IRBuilder<> Builder(BinOp); // pass in the ptr to our BinOp that we want to get values from
