...
for.cond.cleanup:                                 ; preds = %for.inc.lower, %entry.lower
  %new.num.phi.phi = phi i32 [ %new.num.phi, %for.inc.lower ], [ %num, %entry.lower ]
  ret void

for.body.upper:                                   ; preds = %for.inc.lower, %for.body.lr.ph
  %new.0.phi.phi = phi i32 [ %new.0.phi, %for.inc.lower ], [ %0, %for.body.lr.ph ]
  %new.num.phi.phi1 = phi i32 [ %new.num.phi, %for.inc.lower ], [ %num, %for.body.lr.ph ]
...
for.body.upper.saveBB.id0:                        ; preds = %for.body.upper
  ...
  store i32 %new.num.phi.phi1, i32* %store.num_address, align 4
...
for.body.upper.junctionBB.id0:                    ; preds = %for.body.upper.restoreBB.id0, %for.body.upper.saveBB.id0
  %new.num = phi i32 [ %new.num.phi.phi1, %for.body.upper.saveBB.id0 ], [ %loaded.num, %for.body.upper.restoreBB.id0 ]
...
for.cond.cleanup:                                 ; preds = %for.inc.lower, %entry.lower
  %new.num.phi.phi.phi = phi i32 [ %new.num.phi.phi, %for.inc.lower ], [ %num, %entry.lower ]
  ret void
