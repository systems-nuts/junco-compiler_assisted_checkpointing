      ...
      for (j = 0; j < newImageWidth; j++) {
        for (h = 0; h < FILTER_HEIGHT; h++) {
          for (w = 0; w < FILTER_WIDTH; w++) {
            l_sum0 += filter[(h * FILTER_WIDTH) + (w)] *
                      image[(0 * HEIGHT * WIDTH) + ((h + i) * WIDTH) + (w + j)];
            l_sum1 += filter[(h * FILTER_WIDTH) + (w)] *
                      image[(1 * HEIGHT * WIDTH) + ((h + i) * WIDTH) + (w + j)];
            l_sum2 += filter[(h * FILTER_WIDTH) + (w)] *
                      image[(2 * HEIGHT * WIDTH) + ((h + i) * WIDTH) + (w + j)];
          }
        }
